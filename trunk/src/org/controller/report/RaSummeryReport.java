...
public class RaSummeryReport extends ActionSupport implements ServletContextAware{
	...
	{	
		...
		String fileName="OpenUserReport_"+agentId+"_"+dateFormat.format(cal.getTime()).toUpperCase()+".pdf";

		RADAO radao=new RADAO();
		ArrayList<RaProvidedSelectedDTO> rsList=radao.getRaProvidedSelectionList(fromDate,toDate,agentId);
		...
		try{
			...
			RaProvidedSelectedDTO rsDTO=null;
			for(int i=0;i<rsList.size();i++)
			{
				eEvent.setDisplayValue(fromDate+"#"+toDate+"#");
				rsDTO=rsList.get(i);								
				if(i==0){	
					...
					ptable.setWidths(new float[]{10,30,15,15,30});
					...
					pcell=new PdfPCell(new Paragraph("Agent",fontb));
					pcell.setMinimumHeight(25f);
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
					ptable.addCell(pcell);
					
					pcell=new PdfPCell(new Paragraph("Provided Data",fontb));
					pcell.setMinimumHeight(25f);
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
					ptable.addCell(pcell);

					pcell=new PdfPCell(new Paragraph("Provided Date",fontb));
					pcell.setMinimumHeight(25f);
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
					ptable.addCell(pcell);
					
					pcell=new PdfPCell(new Paragraph("Selected Data",fontb));
					pcell.setMinimumHeight(25f);
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
					ptable.addCell(pcell);
				...
				pcell=new PdfPCell(new Paragraph(rsDTO.getCompanyName(),fontt));
				pcell.setMinimumHeight(25f);
				pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
				pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				ptable.addCell(pcell);
				
				pcell=new PdfPCell(new Paragraph(String.valueOf(rsDTO.getProvidedTotal()),fontt));
				pcell.setMinimumHeight(25f);
				pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
				pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				ptable.addCell(pcell);

				pcell=new PdfPCell(new Paragraph(rsDTO.getProvidedDate(),fontt));
				pcell.setMinimumHeight(25f);
				pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
				pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				ptable.addCell(pcell);
				
				pcell=new PdfPCell(new Paragraph(String.valueOf(rsDTO.getSelectedTotal()),fontt));
				pcell.setMinimumHeight(25f);
				pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
				pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				ptable.addCell(pcell);
...
class RaSummeryReportEvent extends PdfPageEventHelper
{
	...
	public void onStartPage(PdfWriter writer, Document document) {
		...
		try{
			...
			String header1="RA Summery Report\n\nFrom Date :"+valArr[0]+", To Date :"+valArr[1];
			pcell = new PdfPCell();
